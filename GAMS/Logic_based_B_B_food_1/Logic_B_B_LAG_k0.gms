...
obj2.. cost =e= - sum(m, sp*produce(m)) + sum((m,p), cost_param(m,p)*buy(m,p)) + sum((m,p), sc*store(m,p))
...
model lagdual /obj2, disj_ineq_hr, bound_up, bound_lo, disj_fix, sum_bin, defusepv,defusepnv,defproduce,defhmin,defhmax,stockbal,final_stock/;

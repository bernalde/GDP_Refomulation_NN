...
y.lo(ki)=0; y.up(ki)=1;
tot_res('HR',level,node,'lb')$(ord(node)=cnt and ord(level)=cnt2)=chull.objval;
chec_int=1;
loop(ki,if((y.l(ki)<0.999 and y.l(ki)>0.001),chec_int=0;););
if(chec_int=1,tot_res('HR',level,node,'ub')$(ord(node)=cnt and ord(level)=cnt2)=chull.objval;);
if(chull.modelstat<>1,tot_res('HR',level,node,'infeas')$(ord(node)=cnt and ord(level)=cnt2)=1;);
cnt=cnt+1;

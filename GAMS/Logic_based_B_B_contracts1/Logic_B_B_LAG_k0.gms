...
*   LBP = smin(numm2$(sum(nnode(numm2,level,kk,ii),1)>=1 and node_stat(numm2)=1 and node_int(numm2)<>1),node_val(numm2));
   ...
   execute_unload "res_B_B_LAG_k0 " tot_results_w_time,tot_results_s_time,first_inter,first_inter_val,best_inter,opt_y,opt_x,max_nod,cnt_int_lag;

);

execute_unload "res_B_B_LAG_k0 " tot_results_w_time,tot_results_s_time,first_inter,first_inter_val,best_inter,opt_y,opt_x,max_nod,check_opt,UBP,LBP,cnt_int_lag;
...
execute_unload "res_prob " sol,nodes,time,LBL,first,best,cnt_int_lag;

...
*                + sum((t,r)$(ord(t)>=2),lambda2(t,r)*(sum((k,i)$logic_left(t,r,k,i),y(k,i)) - sum((tt,k,i)$logic_right(t,r,k,tt,i),y(k,i))))
;
...
model lagdual /obj2, disj_ineq_hr, bound_up, bound_lo, disj_fix, sum_bin,  eq2, eq3, eq4, eq5 /;
...
set numm /1*500000/,nnode(numm,k,k,i);nnode(numm,k,k,i)=no;
...
lambda2(t,r)$(ord(t)>=2) = -eq5.m(t,r);
solve lagdual using rmip min cost;
cnt_int_lag2=0;
loop(ki, if((y.l(ki)>=1-tol and y.l(ki)<=tol),cnt_int_lag2=1););
if(cnt_int_lag2=1,cnt_int_lag=cnt_int_lag+1;);
...
*   LBP = smin(numm2$(sum(nnode(numm2,level,kk,ii),1)>=1 and node_stat(numm2)=1 and node_int(numm2)<>1),node_val(numm2));
   ...
   execute_unload "res_B_B_LAG_k0_2 " tot_results_w_time,tot_results_s_time,first_inter,first_inter_val,best_inter,opt_y,opt_x,max_nod,cnt_int_lag;

);

execute_unload "res_B_B_LAG_k0_2 " tot_results_w_time,tot_results_s_time,first_inter,first_inter_val,best_inter,opt_y,opt_x,max_nod,check_opt,UBP,LBP,cnt_int_lag;
...
execute_unload "res_prob_2 " sol,nodes,time,LBL,first,best,cnt_int_lag;

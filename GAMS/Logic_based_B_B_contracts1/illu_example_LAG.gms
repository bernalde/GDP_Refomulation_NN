...
tot_res('LAG',level,node,'lb')$(ord(node)=cnt and ord(level)=cnt2)=BigM.objval;
if(chec_int=1,tot_res('LAG',level,node,'ub')$(ord(node)=cnt and ord(level)=cnt2)=bigM.objval;);
if(bigM.modelstat<>1,tot_res('LAG',level,node,'infeas')$(ord(node)=cnt and ord(level)=cnt2)=1;);
if(chec_int=0,
   solve lagdual using rmip min cost;
   tot_res('LAG',level,node,'lb')$(ord(node)=cnt and ord(level)=cnt2)=max(lagdual.objval,BigM.objval);
   y.fx(ki)=y.l(ki);
   solve bigM using rmip min cost;
   y.lo(ki)=0; y.up(ki)=1;
   if(bigM.modelstat=1,tot_res('LAG',level,node,'ub')$(ord(node)=cnt and ord(level)=cnt2)=bigM.objval;);
);
y.lo(ki)=0; y.up(ki)=1;
cnt=cnt+1;
